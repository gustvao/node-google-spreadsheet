...
import { getFieldMask, columnToLetter, letterToColumn, checkForDuplicateHeaders } from './utils';
import GoogleSpreadsheet from './GoogleSpreadsheet';
import { A1Range, SpreadsheetId, DimensionRangeIndexes, WorksheetDimension, WorksheetId, WorksheetProperties, A1Address, RowIndex, ColumnIndex, DataFilterWithoutWorksheetId, DataFilter, GetValuesRequestOptions, WorksheetGridProperties, WorksheetDimensionProperties, CellDataRange, AddRowOptions, GridRangeWithoutWorksheetId, GridRangeWithOptionalWorksheetId } from './shared-types';
...
export default class GoogleSpreadsheetWorksheet {
  ...
  private _setProp<T extends keyof WorksheetProperties>(param: T, newVal: WorksheetProperties[T]) {
  ...
  async getRows<T extends Record<string, any>>(
    ...
    for (let i = 0; i < rawRows.length; i++) {
      rows.push(new GoogleSpreadsheetRow<T>(this, rowNum++, rawRows[i]));
    }
    return rows;
  }
